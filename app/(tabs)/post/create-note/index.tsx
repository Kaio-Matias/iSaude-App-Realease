...
export default function CreateNote() {
  ...
  // Limpar estados quando a tela receber foco
  useFocusEffect(
    useCallback(() => {
      // Resetar todos os estados ao entrar na tela
      setText('');
  ...
  const handlePublish = () => {
    ...
    // Salvar post no sistema mockado com todas as mídias selecionadas
    const mediaUris = selectedMedia.length > 0 ? selectedMedia.map(m => m.uri) : undefined;
  ...
  const handleCamera = async () => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      setSelectedMedia(prev => [...prev, { uri: asset.uri, type: mediaType }]);
      console.log('Mídia capturada:', { uri: asset.uri, type: mediaType });
    }
  };
  ...
  const handleGallery = async () => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      const mediaType = asset.type === 'video' ? 'video' : 'image';
      setSelectedMedia(prev => [...prev, { uri: asset.uri, type: mediaType }]);
      console.log('Mídia selecionada:', { uri: asset.uri, type: mediaType });
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Carrossel de Mídias - largura total da tela */}
        {selectedMedia.length > 0 && (
          <View style={styles.imageCarousel}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imagesScroll}>
              {selectedMedia.map((media, index) => (
                <View key={index} style={styles.imageContainer}>
                  <Image source={{ uri: media.uri }} style={styles.carouselImage} />
                  {media.type === 'video' && (
            ...
            <Text style={styles.imageCount}>
              {selectedMedia.length} {selectedMedia.length === 1 ? 'mídia adicionada' : 'mídias adicionadas'}
            </Text>
          </View>
        )}
