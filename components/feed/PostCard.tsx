...
export default function PostCard({ post, onLike, onDelete }: PostCardProps) {
  ...
  // Helper para detectar se é vídeo baseado na extensão
  ...
  const hasVideo = post.images?.some(imageSource => {
        ...
        <View style={styles.imageCarouselContainer}>
          ...
          >
            {post.images.map((imageSource, index) => {
              const uri = typeof imageSource === 'string' ? imageSource : imageSource.uri;
              const mediaIsVideo = isVideo(uri);
              
              if (mediaIsVideo) {
                return (
                  <View key={index} style={styles.postVideo}>
                    <VideoView
...
const styles = StyleSheet.create({
  ...
  postVideo: { 
    width: width - 60,
    height: Math.min(width * 1.8, 700), // Vídeos são 80% mais altos, até no máximo 700px
    borderRadius: 12,
    overflow: 'hidden',
  },
  postMetrics: { 
